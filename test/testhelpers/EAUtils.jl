...
function cache_escapes!(interp::EscapeAnalyzer,
    ...
    caller.argescapes = EscapeCache(argescapes, estate, cacheir)
    return argescapes
end

function getargescapes(mi_cache::MICache) where MICache
    return function (linfo::Union{InferenceResult,MethodInstance})
        if isa(linfo, InferenceResult)
            ecache = linfo.argescapes
        else
            codeinst = CC.get(mi_cache, linfo, nothing)
            isa(codeinst, CodeInstance) || return nothing
            ecache = codeinst.argescapes
        end
        return ecache !== nothing ? (ecache::EscapeCache).argescapes : nothing
    end
end
